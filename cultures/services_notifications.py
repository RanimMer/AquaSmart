...
def envoyer_notifications_cultures_pour_aujourdhui():
    """
    Vérifie toutes les cultures et envoie des emails
    aux petits agriculteurs (TECH) si arrosage ou récolte aujourd'hui.
    Retourne le nombre de fermes pour lesquelles des mails ont été envoyés.
    """
    ...
    for data in notifications_par_ferme.values():
        ...
        destinataires = [
            m.user.email for m in membres if m.user.email
        ]
        ...
        if cultures_arrosage:
            ...
            for c in cultures_arrosage:
                lignes.append(
                    f"  - {c.nom} ({c.get_type_display()}) "
                    f"| besoin: {c.besoin_base_l_j} L/j | semis: {c.date_semis}"
                )
            lignes.append("")
        ...
        if cultures_recolte:
            ...
            for c in cultures_recolte:
                lignes.append(
                    f"  - {c.nom} ({c.get_type_display()}) "
                    f"| récolte prévue: {c.date_recolte_prevue}"
                )
            lignes.append("")
