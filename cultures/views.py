...
def build_weeks(year: int, month: int, events_by_date):
    ...
    weeks = [
      [ {date, in_month, status, cultures}, ... 7 jours ],
      ...
    ]
    status ∈ {"past", "today", "upcoming", ""} pour la couleur.
    """
    weeks = []
    ...
    # On affiche de LUNDI à DIMANCHE : on recule au lundi de la première semaine
    start = first_day - timedelta(days=first_day.weekday())  # weekday(): lundi=0
    # On avance jusqu'au dimanche de la dernière semaine
    ...
    while current <= end:
        ...
        for _ in range(7):
            ...
            cultures_for_day = events_by_date.get(key, [])
...
def calendrier_cultures(request):
    ...
    cultures = _cultures_for_user(request.user)  # tu l’avais déjà

    # 3) Générer les évènements d’irrigation pour ce mois
    events_by_date = generer_evenements_irrigation(cultures, year, month)

    # 4) Construire la grille (weeks) pour le template
    weeks = build_weeks(year, month, events_by_date)

    # 5) Calculer mois précédent / suivant
    prev_month = month - 1
    prev_year = year
    if prev_month == 0:
        prev_month = 12
